...
export function Chart({
  ...
  const select = useRef(onDate);
  useEffect(() => {
    select.current = onDate;
  }, [onDate]);
  useEffect(() => {
    ...
    instance.current = c;
    c.on("click", (p) => {
      if (p.name) select.current?.(p.name);
    });
  ...
  return (
    <div
      ...
      role="img"
      aria-label="График временного ряда. Точные значения доступны в таблице ниже."
    />
  );
}
