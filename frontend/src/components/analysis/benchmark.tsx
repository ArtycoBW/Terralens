  ...
  return (
    <div className="page-pad">
      <div className="page-heading">
        <div>
          <p className="eyebrow">Анонимный конкурсный набор</p>
          <h1>Бенчмарк восстановления</h1>
          <p className="small muted">
            Опубликованные предсказания только для скрытых целевых точек.
          </p>
        </div>
        <span className="pill">Модель {data.model_id}</span>
      </div>
      <div className="metrics">
        <div className="panel">
          <p className="metric-label">Целевых строк</p>
          <p className="metric">{number(data.rows, 0)}</p>
        </div>
        <div className="panel">
          <p className="metric-label">AOI с предсказаниями</p>
          <p className="metric">{data.polygons.length}</p>
        </div>
        <div className="panel">
          <p className="metric-label">Географическая привязка</p>
          <p className="metric text-lg mt-2">Отсутствует</p>
        </div>
        <div className="panel">
          <p className="metric-label">Официальный RMSE</p>
          <p className="metric">—</p>
          <p className="micro muted">Истинные скрытые цели недоступны</p>
        </div>
      </div>
      ...
      <section className="panel stack">
        ...
        {p && (
          <>
            ...
            />
            <div className="scroll-table max-h-[360px]">
              <table className="data-table">
                <thead>
                  <tr>
                    <th>Дата цели</th>
                    <th>primary_ndvi_pred</th>
                  </tr>
                </thead>
                <tbody>
                  {p.points.map((p) => (
      ...
      <section className="panel mt-5 stack">
