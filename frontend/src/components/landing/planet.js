...
export function initPlanet(canvas, onReady = () => {}, onError = () => {}) {
  ...
  function loadPlanet() {
    gltfLoader.load('/assets/planet-lights.glb', (lights) => {
      if (disposed) { disposeObject(lights.scene); return; }
      ...
      gltfLoader.load('/assets/planet.glb', (gltf) => {
        ...
        entryActive = true; entryT = 0; onReady();
      }, undefined, onError)
    }, undefined, onError)
  }
  loadPlanet()
  ...
  const atmoPts = (function () {
    ...
    atmoMat = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uRes: { value: new THREE.Vector2(window.innerWidth * Math.min(window.devicePixelRatio, 1.5), window.innerHeight * Math.min(window.devicePixelRatio, 1.5)) },
      },
      vertexShader: `
  ...
  function render() {
    ...
    if (document.hidden) { t0 = performance.now() / 1000; return; }
    const t = performance.now() / 1000
