...
export function initPlanet(canvas, onReady = () => {}, onError = () => {}) {
  ...
  ambient.layers.enable(LAYERS.ENTIRE_SCENE);
  scene.add(ambient);
  ...
  function addClouds() {
    ...
    for (const layer of CLOUD_LAYERS) {
      const g = new THREE.SphereGeometry(
        CONFIG.planetRadius * CONFIG[layer.hKey],
        64,
        64,
      );
  ...
  function loadPlanet() {
    gltfLoader.load(
      "/assets/planet-lights.glb",
      (lights) => {
        if (disposed) {
          disposeObject(lights.scene);
          return;
        }
        loadedRoots.push(lights.scene);
        const lmesh = firstMesh(lights.scene);
        const nightTex =
          lmesh && lmesh.material && lmesh.material.map
            ? lmesh.material.map
            : null;
        gltfLoader.load(
          "/assets/planet.glb",
      ...
      uniforms: {
        ...
        uRes: {
          value: new THREE.Vector2(
            window.innerWidth * Math.min(window.devicePixelRatio, window.innerWidth < 700 ? 1 : 1.5),
            window.innerHeight * Math.min(window.devicePixelRatio, window.innerWidth < 700 ? 1 : 1.5),
          ),
        },
      },
      vertexShader: `
