...
export type Capabilities = {
  ...
  supported_period: { from: string; to: string };
  active_model: string | null;
  feature_flags: Record<string, boolean>;
  retention: { workspace_days: number; export_days: number };
};
export type Page<T> = {
  items: T[];
...
export async function allPages<T>(
  ...
  do {
    const page: Page<T> = await api<Page<T>>(
    ...
    if (cursor && seen.has(cursor))
      throw new Error("Сервер повторил курсор страницы");
...
export const terminalRun = (state: string) =>
  ["completed", "partial", "no_data", "failed", "cancelled"].includes(state);
export const label: Record<string, string> = {
  ...
  cancelled: "Отменён",
  succeeded: "Готово",
  normal: "Без выявленного стресса",
  stress: "Стресс",
  critical: "Критично",
  insufficient_data: "Недостаточно данных",
  observed: "Наблюдение",
  interpolated: "Интерполяция",
  extrapolated: "Экстраполяция",
  climatology_fallback: "Сезонная оценка",
