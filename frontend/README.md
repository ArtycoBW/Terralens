...
Дополнительные проверки: `node tests/visual-smoke.mjs` (три перехода с освобождением сцен), `node tests/redesign-capture.mjs` (снимки секций и пять ширин). Базовые E2E проверяют контракты, формы, ошибки, клавиатуру, WebGL fallback и навигацию; реальные внешние источники проверяет `node tests/live-flow.mjs`.
